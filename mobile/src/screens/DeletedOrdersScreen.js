...
export default function DeletedOrdersScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      setError('');
      setLoading(true);
      setOrders(await orderService.listDeleted());
    } catch (err) {
    ...
    <Screen>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {!loading && visibleOrders.length === 0 ? (
          <EmptyState title="No hay comandas eliminadas" icon="trash-outline" />
        ) : null}
        {visibleOrders.map((order) => (
          <Card key={order.id}>
            ...
            <Row label="Observacion" value={order.deleteNote || 'Sin observacion'} />
            <AppButton
              title="Ver detalle"
              icon="eye-outline"
              variant="outline"
